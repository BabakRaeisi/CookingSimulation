namespace Mapbox.Example
{
    ...
    public class QuadTreeCameraMovement : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            Spawn_on = GetComponent<SpawnOnMap>();
            if (Spawn_on == null)
            {
                Debug.Log("THE SPAWN MANAGER DOES NOT EXIST!!!!!");
            }
        }
        ...
        public bool pick_destination = false;

        public void activate_destination()
        {

            if (pick_destination == false)
            {
                pick_destination = true;

            }
            else { pick_destination = false; }
        }
        void UseMeterConversion()
        {
            if (Input.GetMouseButtonUp(0) && pick_destination == false && !EventSystem.current.IsPointerOverGameObject())
            {
            ...
            if (Input.GetMouseButton(0) && pick_destination == true)
            {
                var pointScreen = Input.mousePosition;
                pointScreen.z = _referenceCamera.transform.localPosition.y;
                var poss = _referenceCamera.ScreenToWorldPoint(pointScreen);

                var LatLonDelta = _mapManager.WorldToGeoPosition(poss);

                Vector2d MyLatLon = new Vector2d();
                MyLatLon = new Vector2d(LatLonDelta.x, LatLonDelta.y);
                Debug.Log("Latitude: " + LatLonDelta.x + " Longitude: " + LatLonDelta.y);
                Spawn_on.Set_marker(MyLatLon);
            }
            { }
